...
# ⇒ 달력으로 표시(직접 선택) == 출발, 도착일 정해짐 + 일수계산 필요
        if pages.current == 1:
                ...
                air_days=(datetime.strptime(air_end, '%Y-%m-%d')-datetime.strptime(air_st, '%Y-%m-%d')).days
                
                if air_st == air_end :
                  st.markdown("##### 날짜를 선택해주세요")
                elif air_st < air_end :
                  air_days=(datetime.strptime(air_end, '%Y-%m-%d')-datetime.strptime(air_st, '%Y-%m-%d')).days
                  st.markdown("##### {}일".format(str(air_days)))
                else : 
...
# ⇒ 1) 도쿄 2) 오사카 3) 삿포로 4) 오키나와 5) 후쿠오카
        if pages.current == 2:
            ...
            det =survey.radio(
              'det',   options=[1,2,3,4,5], index=0, label_visibility="collapsed"
            )
...
#  1) 인천공항 2) 청주공항 3) 광주공항 4) 대구공항 5)김해공항
        if pages.current == 3:
            ...
            arr =survey.radio(
              'arr',   options=['인천공항' ,'청주공항' ,'광주공항' ,'대구공항', '김해공항'], index=0, label_visibility="collapsed", horizontal=True
            )
...
# ⇒ 1) 상관 없음 2) 이코노미 3)프리미엄 이코노미 4) 비즈니스 5)퍼스트
        if pages.current == 4:
            ...
            st.session_state['air_cls_i'] = air_cls_i
...
# ⇒ 3번 부터 다시 선택
        if pages.current == 5:
            ...
            air_around =survey.radio(
              'air_around', options=['상관 없음' ,'직항만' ], index=0, label_visibility="collapsed", horizontal=True
            )
            air_around_i = dict_around[air_around]
            st.session_state['air_around_i'] = air_around_i
            filtered_df = df[(df['Arr'] == str(st.session_state.get('arr_i'))) & (df['Det'] == str(st.session_state.get('det_i'))) & 
                             (df['grade'] == str(st.session_state.get('air_cls_i')))& (df['time_taken'] == str(air_around_i)) ]
            st.write(filtered_df.head())
            if len(filtered_df)==0:
              st.markdown('조건에 맞는 항공권이 없습니다, 좌석 등급을 바꿔보세요')
