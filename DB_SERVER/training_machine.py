...
class HotelPricePredictor:
    ...
    def load_data(self):
        ...
        df['가장가까운공항과의거리'] = df['가장가까운공항과의거리'].str.replace(' km', '').astype(float)
        df['가장가까운 공항까지시간'] = df['가장가까운공항까지시간'].apply(self.convert_time_to_minutes)
...
class Flight_ModelTrainer:
    ...
    def save_model(self, path):
        ...
        with open(os.path.join(path, 'flight_trained_model.pkl'), 'wb') as file:
            pickle.dump(self.model, file)
...
class CarPricePredictor:
    def __init__(self, car):
        self.car = car
        self.X = self.car.drop(['가격'], axis=1)
        self.y = self.car['가격']
